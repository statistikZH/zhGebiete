...
select_name <- function(list, selection, search_term = NULL) {
  if (is.null(names(list))) {
    ...
    if (is.data.frame(data_array)) {
      # Suche nach einer Spalte, die "code" im Namen hat (z.B. gemeinde_code)
    ...
    if (num_results > 1 && selection) {
      ...
      if (is.data.frame(data_array)) {
        ...
        preview <- do.call(rbind, lapply(data_array, function(x) {
          ...
          res_cols <- res_cols[res_cols != "gebietstyp_code"]
      ...
      # Rückgabe: Das ausgewählte Element (aus dem sortierten Array)
      if (is.data.frame(data_array)) {
