...
[![Codecov test coverage](https://codecov.io/gh/statistikZH/zhGebiete/graph/badge.svg)](https://app.codecov.io/gh/statistikZH/zhGebiete)
<!-- badges: end -->

Das R-Paket `zhGebiete` bietet Nutzenden benutzerfreundliche und standardisierte Abfragen der [Gebietsstammdaten-API](https://gebietsstammdaten.statistik.zh.ch/api/__docs__/#/) aus R.

Die API ermöglicht den einfachen Zugriff auf aktuelle und qualitätsgesicherte Stammdaten der Gemeinden, Bezirke und Raumplanungsregionen des Kantons Zürich.
...
Ein möglicher Use-Case ist das Harmonisieren und Anreichern von Gemeindennamen.
⚠️ Ein Datensatz lässt sich nur validieren, wenn die automatische Suche ein Resultat liefert.

```{r harmonisieren und anreichern, eval=FALSE}
# Pakete laden
library("zhGebiete")
...
```{r basis tabellen abrufen, eval=FALSE}
...
Gemeinde, Bezirk und Raumplanungsregionn-Tabelle lassen sich via Code oder Name filtern. Die Namensabfrage erfolgt dabei harmonisiert.

```{r filtern gemeinde region und rpr, eval=FALSE}
...
```{r filtern gemeindezuweisung, eval=FALSE}
...
Jahresstände von Gemeinden lassen sich nach Gemeindecode und Jahr filtern.

```{r filtern gemeindehist, eval=FALSE}
